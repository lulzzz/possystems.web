...
export const ReportIcon = withStyles(styles)(({ classes, ...props }) => (
  <Icon className={classes.reporticon} {...props} />
));
...
class DailySalesReport extends Component {
  ...
  handleClick = query => {
    ...
    axios
      .get('api/dailysalesreport/file' + this.buildQueryString('?', query), {
        responseType: 'blob',
      })
      .then(function(res) {
        if (res.headers['content-type'] == 'application/ms-excel') {
          fileDownload(res.data, 'Daily Sales Report.xlsx');
        } else if (res.headers['content-type'] == 'application/pdf') {
          fileDownload(res.data, 'Daily Sales Report.pdf');
        }
      })
      .catch(e => {
  ...
  render() {
    return this.state.permitted ? (
      <Card>
        <Title title="Daily Sales Report" />
        <CardContent>
          <form>
            <TextField
              id="date"
