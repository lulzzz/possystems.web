...
class DetailedSalesReport extends Component {
  ...
  buildQueryString = (prefix, pdf) => {
    ...
    return `${prefix}terminalid=${terminalId}&userid=${userId}&supplierid=${supplierId}&startDate=${this.state.minDate}&endDate=${this.state.maxDate}${pdf}`;
  };
  ...
  handleClick = query => {
    ...
    axios
      .get('api/detailedsalesreport/file' + this.buildQueryString('?', query), {
        responseType: 'blob',
      })
      .then(function(res) {
        if (res.headers['content-type'] == 'application/ms-excel') {
          fileDownload(res.data, 'Detailed Sales Report.xlsx');
        } else if (res.headers['content-type'] == 'application/pdf') {
          fileDownload(res.data, 'Detailed Sales Report.pdf');
        }
      })
      .catch(e => {
  ...
  render() {
    return this.state.permitted ? (
      <Card>
        <Title title="Detailed Sales Report" />
        <CardContent>
          <form>
            <TextField
              id="date"
